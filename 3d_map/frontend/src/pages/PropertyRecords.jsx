    ...
    <div className="max-w-[1080px]">
      ...
      <div className="flex items-center gap-2 mt-4">
        <div className="relative flex-1 max-w-[420px]">
          ...
          <input
            className="w-full bg-surface border border-line rounded-[10px] pl-9 pr-3 py-2 text-sm text-ink placeholder-ink-soft outline-none focus:border-blue"
        ...
        <button
          className="w-9 h-9 grid place-items-center bg-surface border border-line rounded-[10px] text-ink-mid hover:text-ink"
          aria-label="Filters"
        >
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border ${
              filter === c.key
                ...
                : 'bg-surface text-ink-mid border-line hover:text-ink'
            }`}
          >
            {c.label}
          </button>
      ...
      <div className="bg-surface border border-line rounded-[14px] p-3 mt-4">
        ...
        {rows.map((r, i) => (
          <MiniRow
            ...
            pill={<StatusPill variant={r.status === 'verified' ? 'verified' : 'review'}>{r.status}</StatusPill>}
